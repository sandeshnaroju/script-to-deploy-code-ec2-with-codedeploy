...
DEPLOY=$(aws deploy create-deployment --application-name <codedeploy_app_name> --s3-location bucket=<bucket_name>,key=<project_name>-$1.zip,bundleType=zip,eTag=$ETAG --ignore-application-stop-failures --deployment-group-name <codedeploy_deployment_group_name> --deployment-config-name CodeDeployDefault.OneAtATime --description "$2")
wait
